...
for i, file in enumerate(Path.joinpath(cwd, 'Si(001)-AsH3 examples', '50nm').iterdir()):
    if file.suffix != '.mtrx':
        print(file)
        # load matrix files
        Si_scans[f'Si(001)-AsH3_25nm_{i}'] = nvm.STM( str(file) , None, None, None, standard_pix_ratio=512/25)
        # plane level and scan line align and correct hysterisis
        #if Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_up:
        Si_scans[f'Si(001)-AsH3_25nm_{i}'].clean_up(Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_up, 'trace up', plane_level=True)
        Si_scans[f'Si(001)-AsH3_25nm_{i}'].clean_up(Si_scans[f'Si(001)-AsH3_25nm_{i}'].retrace_up, 'retrace up', plane_level=True)
        Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_up_proc, Si_scans[f'Si(001)-AsH3_25nm_{i}'].retrace_up_proc, corrected = Si_scans[f'Si(001)-AsH3_25nm_{i}'].correct_hysteresis(Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_up_proc, Si_scans[f'Si(001)-AsH3_25nm_{i}'].retrace_up_proc, 'trace up')       
    #if Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_down:
        #    Si_scans[f'Si(001)-AsH3_25nm_{i}'].clean_up(Si_scans[f'Si(001)-AsH3_25nm_{i}'].retrace_up, 'retrace up', plane_level=True)
        #    Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_down_proc, Si_scans[f'Si(001)-AsH3_{i}'].retrace_down_proc, corrected = Si_scans[f'Si(001)-AsH3_25nm_{i}'].correct_hysteresis(Si_scans[f'Si(001)-AsH3_{i}'].trace_down_proc, Si_scans[f'Si(001)-AsH3_{i}'].retrace_down_proc, 'trace up')       
        fwd = Si_scans[f'Si(001)-AsH3_25nm_{i}'].trace_up_proc
        bwd = Si_scans[f'Si(001)-AsH3_25nm_{i}'].retrace_up_proc
        Si_scans_FSL[f'Si(001)-AsH3_25nm_{i}'] = fsl_ui.FSL_Scan(str(file)[100:-6],'Si', fwd, bwd, size=25, custom_res = 1024)
